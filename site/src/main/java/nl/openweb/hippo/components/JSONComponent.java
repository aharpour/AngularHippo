...
public class JSONComponent extends BaseHstComponent {
	...
	private Gson gson = HstServices.getComponentManager().getComponent(
			"gsonExposeAnnotation");
	...
	private Map<String, Object> getModel(HstRequest request) {
		...
		result.put("template", request.getRequestContext()
				.getResolvedSiteMapItem().getHstComponentConfiguration()
				.getRenderPath());
